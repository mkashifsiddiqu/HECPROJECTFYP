...
import {
    Alert,Box,Checkbox,FormControlLabel,FormGroup,createTheme, ThemeProvider,
    Grid,Table,TableBody,TableCell,TableHead,TableRow,Typography,Divider,TableContainer
} from '@mui/material';
...
const VerifyDetails = () => {
  ...
  const [postalcode, setpostalcode] = useState<string>(``)
    ...
    const getMode =async() =>{
      const  data={email}
        const res = await fetch(`http://localhost:3000/api/Student/Application/Mode/getMode`, {
            ...
            <ThemeProvider theme={theme}>
                ...
                <Box>
                    ...
                    <Table size="small" sx={{ margin: `0 0 20px 0` }}>
                        ...
                        <TableBody>
                            <TableRow>
                                ...
                                <TableCell>
                                    <Typography fontSize={13} fontWeight={700} color={`#212529`} textTransform={`capitalize`}>
                                        {` `}
                                       {name}
                                    </Typography>
                                </TableCell>
                    ...
                    <FormGroup>
                        <FormControlLabel
                            ...
                            label={<Typography fontSize={`13px`} fontWeight={600} color={`rgba(51,51,51,.85)!important`}>Please ensure that there is no variation in your name, father's name and date of birth written in personal details as mentioned above</Typography>}
                        />
                    </FormGroup>
                ...
                <Box marginTop={`40px`}>
                    ...
                    <FormGroup>
                        <FormControlLabel
                            control={<Checkbox required color="success" />}
                            label={<Typography fontSize={`13px`} fontWeight={600} color={`rgba(51,51,51,.85)!important`}>I Mr./Ms. {name} User bearing {identityType} # {identityNumber} hereby solemnly declare that all my degree(s)/Transcript(s)/Certificates including Secondary School Certificate (SSC), Higher Secondary Certificate (HSSC) or Equivalent and all my subsequent degrees/transcripts or equivalent are genuine from recognized university/degree awarding institute and are in line with academic standards. If at any stage it is revealed that any of my academic degree(s)/Transcript(s) /Certificates are FAKE/BOGUS or not in line with academic regulations of the university, the HEC would have right to immediately cancel the attestation done on my degree(s)/Transcript(s)/Certificates. Besides, I would be held accountable for disciplinary and legal proceedings for concealment of facts and forgery of academic documents.</Typography>}
                        />
                    </FormGroup>
