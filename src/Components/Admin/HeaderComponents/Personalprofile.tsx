...
export default function BasicMenu() {
  ...
  const list = useSelector((state)=>state.loginuserReducer.userData)
  const {name,email,page} = list.data
  // const name=`dump`,email=`dump`;
  return (
    <>
    <div>
      <Button disableRipple onClick={handleClick} sx={{ textTransform: `none`, '&:hover': { backgroundColor: `transparent` } }}><Typography display={`flex`} alignItems={`center`} fontFamily={`montserrat`} fontWeight={600} fontSize={13} color="#636177">Greetings, {name}<Avatar sx={{ marginLeft: `0.55rem` }}>A</Avatar></Typography></Button>
      <Menu
        id="basic-menu"
        anchorEl={anchorEl}
        open={open}
        ...
        <MenuItem sx={{ padding: `20.8px 19.5px` }} onClick={handleClose}>
          <Box display={`flex`} alignItems={`center`}>
            ...
            <motion.div
              ...
              transition={{ type: `tween` }}
            >
            <Box onClick={() => {router.push(`/Admin`)
            removeCookies(`nameAdmin`) 
            removeCookies(`emailAdmin`) 
            removeCookies(`pageAdmin`)
          }}>
